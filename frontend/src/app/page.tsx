...
export default function PauliHome() {
      ...
      <div className="owner-shell min-h-screen text-[#171714] flex items-center justify-center p-6">
        <div className="owner-card w-full max-w-md rounded-[2.2rem] p-8 md:p-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] font-semibold tracking-[0.26em] uppercase text-black/40">Pauli&apos;s Place</div>
              <div className="text-sm font-semibold">Private owner access</div>
            </div>
          </div>
          ...
          <form onSubmit={onAuth} className="mt-8 space-y-3">
            <input type="email" required value={authEmail} onChange={(event) => setAuthEmail(event.target.value)} placeholder="you@example.com" className="w-full rounded-2xl border border-black/10 bg-white/70 px-4 py-3.5 text-sm outline-none focus:border-black/30" />
            <button className="w-full rounded-2xl bg-[#171714] px-4 py-3.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-black">Send secure sign-in link</button>
          </form>
  ...
  return (
    <div className="owner-shell min-h-screen text-[#171714] pb-24 md:pb-10">
      <header className="owner-topbar sticky top-0 z-30 border-b border-black/[0.055]">
        <div className="mx-auto flex max-w-[86rem] items-center justify-between px-5 py-4 md:px-8 lg:px-10">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-[0.24em] text-black/38">Pauli&apos;s Place</div>
          ...
          <button onClick={async () => { await signOut(); setAuthRequired(true); setOverview(null); }} className="grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white/65 text-black/45 transition hover:bg-white hover:text-black" aria-label="Sign out"><LogOut className="h-4 w-4" /></button>
        </div>
      </header>
          ...
          <div id="pauli" className="pauli-stage rounded-[2.25rem] p-6 text-white md:p-9 lg:p-11">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <div className="text-[10px] font-semibold uppercase tracking-[0.24em] text-white/42">Pauli</div>
                    <span className="h-1 w-1 rounded-full bg-white/25" />
                    <div className="text-[10px] uppercase tracking-[0.16em] text-white/35">Executive layer</div>
                  </div>
            ...
            <form onSubmit={onMission} className="relative z-10 mt-8 space-y-3">
              <div className="relative">
                ...
                <button type="button" onClick={toggleListening} aria-label="Talk to Pauli" className={`absolute bottom-4 right-4 grid h-12 w-12 place-items-center rounded-full bg-[#f4efe4] text-black shadow-lg transition hover:scale-[1.04] ${listening ? 'ring-4 ring-[#d7c39b]/20' : ''}`}>{listening ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}</button>
              </div>
              <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                ...
                <div className="flex gap-2">
                  ...
                  <button disabled={!canSubmit} className="flex min-w-32 items-center justify-center gap-2 rounded-2xl bg-[#f4efe4] px-5 py-3.5 text-sm font-semibold text-black shadow-lg transition hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-30">{submitting ? 'Starting…' : 'Go'} <Send className="h-4 w-4" /></button>
                </div>
              </div>
            </form>
            <div className="relative z-10 mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-white/32">
              <span>Reversible work runs autonomously.</span>
              <span className="hidden h-1 w-1 rounded-full bg-white/20 sm:block" />
              <span>Production, public sends, and consequential spend come back to you.</span>
            </div>
          </div>
                ...
                <div key={approval.id} className="owner-card approval-card rounded-[1.65rem] p-5 md:p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="mt-2 text-xl font-semibold tracking-[-0.035em]">{humanize(approval.action_class)}</h3>
                      <p className="mt-2 max-w-xl text-sm leading-6 text-black/50">{approval.max_spend_cents ? `Maximum authorized spend: ${money(approval.max_spend_cents)}.` : 'No spend is authorized by this approval.'} Scope stays limited to this action.</p>
                    </div>
                  ...
                  <div className="mt-5 flex gap-2">
                    <button onClick={() => void decide(approval, 'deny')} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl border border-black/10 bg-white/45 px-4 py-3 text-sm font-semibold transition hover:bg-white"><X className="h-4 w-4" /> Decline</button>
                    <button onClick={() => void decide(approval, 'approve')} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[#171714] px-4 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5"><Check className="h-4 w-4" /> Approve</button>
                  </div>
                </div>
              ))}
            ...
            <div className="owner-card mt-4 rounded-[1.65rem] p-5 md:p-6">
              <div className="space-y-1">
                {activeAgents.slice(0, 5).map((agent, index) => (
                  <div key={agent.id} className="flex items-center justify-between gap-4 rounded-xl px-1 py-3">
              ...
              <Link href="/agents" className="mt-4 flex min-h-12 items-center justify-between border-t border-black/[0.06] pt-4 text-sm font-semibold">See workforce <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </section>
        </section>
          ...
          <section>
            ...
            <div className="owner-card mt-4 overflow-hidden rounded-[1.65rem]">
              {(overview?.missions ?? []).slice(0, 5).map((mission) => (
                <Link key={mission.id} href="/missions" className="group flex min-h-16 items-center justify-between gap-4 border-b border-black/[0.055] p-4 last:border-0 hover:bg-white/55">
...
function TopNav({ href, label, active = false }: { href: string; label: string; active?: boolean }) {
  return <Link href={href} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${active ? 'bg-[#171714] text-white shadow-sm' : 'text-black/45 hover:bg-white/70 hover:text-black'}`}>{label}</Link>;
}

function BottomNav({ href, label, icon: Icon, active = false }: { href: string; label: string; icon: any; active?: boolean }) {
  return <Link href={href} className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-semibold transition ${active ? 'bg-[#171714] text-white' : 'text-black/45 active:bg-black/[0.04]'}`}><Icon className="h-4 w-4" />{label}</Link>;
}
