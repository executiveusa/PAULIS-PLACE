# BARS + Trail Mixx + Fleet Observability — Execution Plan

## Goal
Make the five-agent architecture operational, with BARS as the public-facing 3D operator and Trail Mixx as BARS's music/radio domain, while PAULIS-PLACE remains the canonical mission/evidence control plane and observer surfaces show truthful runtime state.
...
2. BARS has a production landing/front door that uses a BARS-specific interactive 3D body, not a generic stock robot.
3. On desktop, BARS reacts to pointer movement; on mobile, touch targets on his body are large enough to use and invoke real bounded actions.
4. Each visible BARS hotspot is mapped to a real backend capability or a truthful unavailable state; no fake/demo success data.
5. BARS can create/read Trail Mixx work through an adapter contract without making AzuraCast/Trail Mixx itself the agent brain.
6. Trail Mixx has its own landing/product surface connected to the same bounded BARS music/radio actions.
...
8. Hermes can delegate a bounded Trail Mixx mission to BARS and receive a verified result.
...
- **BARS** — bounded operator; computer/media/music execution; Trail Mixx character/product.
...
- **Trail Mixx** — product/domain system for radio/music. Current source repo is `executiveusa/trail-mixx-source-code`, a fork of AzuraCast (AGPL-3.0).
...
- Inspect BARS, Trail Mixx, PAULIS-PLACE, current Vercel deployment and canonical Supabase state.
- Record duplicate/legacy identities and existing runtime authority.
- Define BARS <-> Trail Mixx adapter contract before implementation.

**Exit proof:** exact repos/branches/deployments/contracts documented; no unresolved ambiguity about source of truth.

### Phase 2 — BARS 3D front door
- Replace generic/legacy presentation with BARS-specific 3D body.
- Prefer self-contained Three.js/procedural BARS if no owned Spline scene exists; Spline can be used only with an owned BARS scene.
...
- Frontend talks to existing BARS backend; it does not duplicate agent logic.

Initial hotspot contract:
- head/visor: TALK / voice
- chest: MISSION / chat
- left arm: TRAIL MIXX / radio control
...
### Phase 3 — Trail Mixx adapter + landing page
- Treat `trail-mixx-source-code` as the Trail Mixx source product and radio engine.
- Do not merge the entire AGPL codebase into BARS.
- Add a narrow `trailmix` adapter/tool contract to BARS for station status, now playing, playlists/queues, scheduling and bounded radio operations supported by the actual Trail Mixx/AzuraCast API.
- Build a Trail Mixx landing/product surface that introduces BARS as the operator.
- Separate read actions from writes; writes require appropriate mission approval.

**Exit proof:** BARS reads real Trail Mixx station state and completes one harmless read-only mission with evidence.
...
- Deploy BARS and Trail Mixx public surfaces.
...
3. BARS 3D front door and real hotspot API mapping.
4. Trail Mixx adapter and landing page.
...
- No destructive Trail Mixx action without explicit policy/approval.
- No dependency on a specific music generation vendor in the BARS identity; vendor adapters remain replaceable.
- AGPL obligations for the Trail Mixx/AzuraCast fork must be preserved.
