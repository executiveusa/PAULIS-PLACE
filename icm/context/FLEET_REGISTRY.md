...
| **TARS** | Operator / music character | Execute bounded computer/media/music missions and power Trail Mix | Company brain, personal-memory owner, or unrestricted external actor |
...
Personal intent defaults to **Pi**. Business/project/client/revenue intent defaults to **Hermes**. Explicit music/media/operator intent may be delegated by Hermes or invoked through **TARS**. **Lightning** receives observation events but does not become the mission owner. **Jarvis** is the front door, not the brain.
...
Pi's private context is not automatically readable by Hermes, TARS, public observers, or free/random model routes. Cross-boundary sharing requires an explicit scoped handoff or an owner-approved summary.
...
## TARS — Operator / Trail Mix

TARS is the high-agency public operator and music/media specialist. It may use computer control and vendor adapters for music, radio, audio, video, browser work, and Trail Mix experiences. Vendor-specific systems such as music-generation platforms sit behind stable tool contracts so the persona is not coupled to one provider.

TARS is a bounded experiment/product character. High-consequence external actions still obey PAULIS-PLACE approval policy.
