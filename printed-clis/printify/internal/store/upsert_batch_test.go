...
// TestUpsertBatch_KeysConnectionJsonByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysConnectionJsonByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "shops_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "shops_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "shops_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "shops_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("connection_json", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("connection_json count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "connection_json")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("shops_id=parent_A count = %d, want 2", parentMatches)
	}
}
...
func TestUpsertBatch_KeysOrdersByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "orders")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_KeysOrdersJsonByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysOrdersJsonByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("orders_json count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "orders_json")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_KeysProductsByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "products")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_KeysProductsJsonByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysProductsJsonByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("products_json count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "products_json")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_KeysWebhooksByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysWebhooksByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("webhooks count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "webhooks")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_KeysWebhooksJsonByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysWebhooksJsonByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("webhooks_json count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "shops_id" = ?`, "webhooks_json")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by shops_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_KeysArchiveJsonByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "uploads_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "uploads_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "uploads_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "uploads_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("archive_json", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "uploads_id" = ?`, "archive_json")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by uploads_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("uploads_id=parent_A count = %d, want 2", parentMatches)
	}
}
