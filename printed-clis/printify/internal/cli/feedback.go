...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRINTIFY2_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRINTIFY2_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "printify2-pp-cli/feedback")
...
When ` + "`PRINTIFY2_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRINTIFY2_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printify2-pp-cli feedback list
  printify2-pp-cli feedback list --limit 5
  printify2-pp-cli feedback list --json`,
