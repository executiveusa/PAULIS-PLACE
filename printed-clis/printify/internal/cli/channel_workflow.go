...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  printify2-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  printify2-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"catalog", "catalog-print-providers-json", "shops-json"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  printify2-pp-cli workflow status

  # Show status as JSON
  printify2-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("printify2-pp-cli")
			}
