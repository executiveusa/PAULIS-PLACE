...
Run sync first to populate the local search index.`,
  ...
  printify2-pp-cli search "status"

  # Force local search explicitly
  printify2-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  printify2-pp-cli search "status" --type catalog --data-source local
  # JSON output for piping
  printify2-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'printify2-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
