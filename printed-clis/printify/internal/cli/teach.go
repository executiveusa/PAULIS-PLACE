...
// printify2-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"printify2-pp-cli/internal/learn/entities"
	"printify2-pp-cli/internal/learn/lookups"
	"printify2-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "PRINTIFY2_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("printify2-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  printify2-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  printify2-pp-cli recall "<question>" --agent
  printify2-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printify2-pp-cli learnings list --agent
  printify2-pp-cli learnings list --query "<substring>"
  printify2-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  printify2-pp-cli learnings forget "<question>" --resource <id>
  printify2-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  printify2-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  printify2-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
