...
type Config struct {
	...
	Printify2BearerAuth string          `toml:"bearer_auth"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("PRINTIFY2_BEARER_AUTH"); v != "" {
		cfg.Printify2BearerAuth = v
		cfg.markEnvOverride("Printify2BearerAuth")
		cfg.AuthSource = "env:PRINTIFY2_BEARER_AUTH"
		cfg.CredentialSource = "env:PRINTIFY2_BEARER_AUTH"
	}
	...
	if v := os.Getenv("PRINTIFY2_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("PRINTIFY2_SHOP_ID")); v != "" {
		cfg.TemplateVars["shop_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["shop_id"] = "shop_id_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PRINTIFY2_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "printify2-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.Printify2BearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PRINTIFY2_BEARER_AUTH"
		}
		return "Bearer " + c.Printify2BearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		Printify2BearerAuth: c.Printify2BearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.Printify2BearerAuth == "" {
		c.Printify2BearerAuth = creds.Printify2BearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.Printify2BearerAuth = ""
	delete(c.envOverrides, "Printify2BearerAuth")
	c.updateFileConfigField("Printify2BearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["Printify2BearerAuth"] {
			out.Printify2BearerAuth = c.fileConfig.Printify2BearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "Printify2BearerAuth":
		c.fileConfig.Printify2BearerAuth = c.Printify2BearerAuth
	}
}
