# Printify2 Printed CLI Agent Guide

This directory is a generated `printify2-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
printify2-pp-cli doctor --json
printify2-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
printify2-pp-cli which "<capability>" --json
printify2-pp-cli <command> --help
```
...
```bash
printify2-pp-cli <command> --agent
```
...
```bash
printify2-pp-cli <command> --help
printify2-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `printify2-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `PRINTIFY2_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
